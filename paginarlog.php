...
include 'conexionPHP.php';
	...
	$sth = $conn->prepare('SELECT count(*) as total from tblog');
	$sth->execute();
...
$consulta="SELECT idlog,texto,fecha from tblog LIMIT $inicio,$TAMANO_PAGINA" ;
...
/*$consulta="SELECT Nombre, idusuario FROM tbusuarios group by Nombre";
$query = $conn->prepare($consulta);
$query->execute();		
...
echo '<h1>Usuarios</h1>'; 
echo '<select name="a">';
foreach($conn->query('SELECT Nombre,Idusuario FROM tbusuarios group by Nombre') as $row) {
